...
CategoryList AdTargeting::GetWinningCategories() const {
  ...
  if (features::IsBaselineClassifierEnabled()) {
    contextual_winning_categories =
        GetBaselineClassificationWinningCategories();
  } else {
  ...
  // behavioural segments should still get shuffled in.
...
CategoryList AdTargeting::GetBaselineClassificationWinningCategories() const {
  return ads_->get_baseline_classifier()->GetWinningCategories();
}
