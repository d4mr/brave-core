...
const base::Feature kBaselineClassifier { "BaselineClassifier",
    base::FEATURE_DISABLED_BY_DEFAULT };

bool IsBaselineClassifierEnabled() {
  return base::FeatureList::IsEnabled(kBaselineClassifier);
}
...
void Log() {
  ...
  if (IsBaselineClassifierEnabled()) {
    BLOG(1, "Baseline Classifier enabled");
  } else {
    BLOG(1, "Baseline Classifier disabled");
  }
